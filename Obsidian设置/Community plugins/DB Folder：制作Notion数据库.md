它可以用来制作和 Notion 一样的数据库，实时拖拽列宽、行宽，在表格视图中修改数据时自动映射到笔记属性本身，几乎和 Notion 数据库一样。不过我还是建议后期迁移到使用 [[Dataview]] 创建数据库

一是 DB Folder 插件本身存在一点 bug。我发现的是从 csv 文件导入功能不能正常使用，还有导出时有乱码。导出不能用就有点鸡肋了
