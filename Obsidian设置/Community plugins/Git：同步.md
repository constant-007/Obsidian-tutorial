...
4. 为实现同步，我们需要有一个远程Git仓库，这里选择GitHub作为仓库托管平台 ^crate-github-repo
	...
	2. 下面需要将本地仓库与这个新建的远程仓库相关联。同样有两种方法，通过git插件命令面板或者命令行实现
		1. 这是使用Git插件命令面板的方法：
			...
			- 接下来需要指定本地分支跟踪的远程分支：继续打开Command palette，找到”Git: Set upstream branch"命令并运行
...
2. 打开新创建的**空仓库**，在第三方社区插件商店中[[Git：同步#^ded78c|安装git插件]]并启用
...
> [!important]+ 关于在移动端操作的注意事项
> 1. 在移动端调出命令面板的方法是在文件中从顶部往下滑
> 2. 在移动端克隆私人仓库会要求你输入GitHub用户名和密码/personal access token，这里需要注意用personal access token，用密码会没反应。应该在电脑上操作也需要，只不过我电脑上早就配置好，上面写教程的时候没弹出这个提示也就忘了这一点，初次使用Git的小伙伴注意一下哈
> 3. 不知道是我设备性能不行还是怎么，每次我在移动端克隆完程序直接就闪退了，重新打开也不行。所以这里建议使用play商店中的gitsync这个软件实现下载远程Obsidian库


如果你想直接用我的这个仓库，不想自己完全重新配置仓库，可以这样做：

1. 首先，按上面的步骤[[Git：同步#^596f07|让你的Obsidian库追踪我的GitHub仓库]]
2. 确保你已经[[Git：同步#^crate-github-repo|]创建了一个私有GitHub仓库]]
3. 在Command palette中找到"Git: Remove remote“命令并运行
4. 最后，按上面的步骤将你新建的私有仓库[[Git：同步#^track-upstream|设置为远程仓库并跟踪特定分支]]
...
![](https://img.aiexplorernote.com/%E4%BD%BF%E7%94%A8Git%E6%8F%92%E4%BB%B6%E8%AE%A9Obsidian%E5%AE%9E%E7%8E%B0%E5%90%8C%E6%AD%A5/git-settings-in-obsidian-4.jpg)
...
![](https://img.aiexplorernote.com/%E4%BD%BF%E7%94%A8Git%E6%8F%92%E4%BB%B6%E8%AE%A9Obsidian%E5%AE%9E%E7%8E%B0%E5%90%8C%E6%AD%A5/git-settings-obsidian-5.jpg)
