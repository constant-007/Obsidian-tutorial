...
2. **拼写检查语言 (Spellcheck languages)：** 选择拼写检查器使用的语言。
    - 在图片中，当前设置为 "English (United States)"。
    - 你可以点击该区域来添加或更改语言，确保在不同语言的笔记中都能获得准确的拼写建议。
3. **自动配对括号 (Auto-pair brackets)：** 自动补全括号和引号。
    
    - 启用后，当你输入一个开括号（如 `(`, `[`, `{`）或开引号（如 `'`, `"`）时，编辑器会自动为你输入相应的闭括号或闭引号。
    - 这有助于提高打字效率并减少括号或引号未闭合的错误。
4. **自动配对 Markdown 语法 (Auto-pair Markdown syntax)**: 自动为 Markdown 特定语法字符进行配对。
    
    - 例如，当你输入一个星号 `*` 用于强调或列表时，编辑器可能会根据上下文自动补全另一个星号，或者在你输入反引号 `` ` `` 来创建行内代码时自动补全。
5. **智能列表 (Smart lists)**: 自动处理列表的缩进和项目符号。
    
    - 开启此功能后，在你创建列表（例如，以 `-` 或 `1.` 开头）并按回车换行时，新的一行会自动继承列表格式，并进行正确的缩进和编号/项目符号。
6. **使用 Tab 缩进 (Indent using tabs)**: 设置按下 `Tab` 键时插入的是制表符还是空格。
    
    - 开启此选项：按下 `Tab` 键会插入一个实际的制表符（tab character）。
